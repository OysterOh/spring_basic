...
public class SnsBoardController {
	...
	/*
	  게시글의 이미지 파일 요청
	  img 태그의 srt 속성을 통해 요청이 들어온다.
	  snsList.jsp 페이지가 로딩되며, 글 목록을 가져오고, JS를 이용해서 화면을 꾸밀 때
	  img 태그에 src에 작성된 요청 url을 통해 자동으로 요청이 들어온다.
	  요청을 받아주는 메서드를 선언하여 경로에 지정된 파일을 보낼 것이다.
	  */
	...
	public ResponseEntity<byte[]> getFile(@PathVariable String fileLoca, @PathVariable String fileName) {
		...
		//응답내용, 응답 성공 여부, 응답 관련 여러 설정들을 지원한다.
		...
		try {
			...
			//사용자에게 보여주려는 데이터가 어떤 파일인지에 따라 응답 상태 코드를 다르게 리턴하고
