...
public class ReplyController {
	...
	public Map<String, Object> getList(@PathVariable int bno, @PathVariable int pageNum) {
		...
		/*
		 1. getList 메서드가 글 번호, 페이지 번호를 경로에서 떼온다.
		 2. Mapper 인터페이스에 복수의 값을 전달하기 위해 Map/@Param을 쓸지 선택
		 3. ReplyMapper.xml에 sql문을 페이징 쿼리로 작성
		 4. 클라이언트 측으로 DB에서 조회한 댓글 목록을 보낼 때, 
		 페이징을 위한 댓글의 총 개수도 같이 보내줘야 한다.
		 복수개의 값을 리턴하기 위해서 리턴타입을 Map으로 줄 지, VO형식으로 줄지 결정해야한다.
		...
		map.put("total", total);	//게시글에 달린 댓글 총 개수
		return map;
	}
