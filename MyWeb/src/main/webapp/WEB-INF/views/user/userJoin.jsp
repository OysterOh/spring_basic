...
<script>
	...
	document.getElementById('idCheckBtn').onclick = function () {
		...
		*/
		...
		- Promise 객체를 자동으로 리턴해서 손쉽게 통신의 응답데이터를 소비할 수 있게 해준다.
		  (Promise: 비동기 통신의 순서를 보장하는 문법)
		- fetch 함수가 리턴하는 Promise 객체는 단순한 응답 JSON 데이터가 아닌
		  전체적이고, 포괄적인 응답 정보를 가지고 있다.
		- 따라서, 서버가 응답한 여러 정보 중 JSON 데이터만 소비하려면 json()이라는 메서드를 사용한다.
		...
		*/
		...
		//Promise객체의 상태가 요청 성공일 시 데이터 후속 처리 진행
		.then(res => {
			//fetch 함수를 통해 비동기 통신이 실행되고, 
			//요청이 완료된 후 then()의 매개값으로 응답에 관련된 함수를 콜백 방식으로 전달한다.
			//함수의 매개변수를 선언하면 해당 매개변수로 
			//응답에 관련된 전반적인 정보를 가진 응답 정보가 전달된다.
			// console.log(res);
		...
		*/
		...
		fetch('${pageContext.request.contextPath}/user/idCheck', reqObj)
			.then(res => res.text()) //요청 완료 후 응답 정보에서 텍스트만 빼기
				.then(data =>  {  //텍스트만 뺀 Promise 객체로부터 data 전달받는다.
					if(data === 'available') {
						//더이상 아이디를 작성할 수 없도록 막는다.
	...
	document.getElementById('mail-check-btn').onclick = function() {
		...
		fetch('${pageContext.request.contextPath}/user/mailcheck?email=' + email).then()
	}
