...
<body>
	...
	<script>
		...
		function regist() {
			...
			//세션에서 현재 로그인중인 사용자의 정보(아이디)를 얻어오자
			...
			대부분 form 태그 이용해서 전송한다.
			*/
			const formData = new FormData();
			const $data = document.getElementById('file'); //img 첨부 input 태그

			console.log('data: ' + $data);

			//한 화면에 파일 업로드 버튼이 여러개일 경우 요소의 인덱스를 지목해서 가져올 수 있다.
			//파일 첨부 버튼 하나고, id를 지목해서 가져오므로 인덱스 필요없다.
			// console.log('data[0]: ' + $data[0]);

			console.log($data.files); //파일 태그에 담긴 파일 정보를 확인하는 프로퍼티
			console.log($data.files[0]);
			//사용자가 등록한 최종 파일의 정보
			//여러 파일을 하나의 태그로 받을 수 있도록 multiple을 제공했다면
			//files -> FileList에 여러 파일의 정보가 들어오게 된다.
			//FileList는 유사배열이므로 인덱스를 이용해서 여러 파일 중 하나를 지목할 수 있다.
			//지금 multiple이 없으므로 [0]이라고 작성하면 파일 정보를 얻는다.
			...
			//FormData 객체를 보낼 때는 따로 headers 설정을 하지 않는다.
				...
				.then(data => {
					...
					for (vo of data) {
						...
						<div class="image-inner">
							<!-- 이미지영역 -->
							<a href="` + vo.bno + `">
								<img data-bno="` + vo.bno + `" src="${pageContext.request.contextPath}/snsboard/display/` + vo.fileLoca + `/` +
							vo.fileName + `">
							</a>
						</div>
		...
		document.getElementById('contentDiv').addEventListener('click', e => {
			...
			//fetch함수를 사용하여 글 상세보기 요청을 비동기식으로 요청
			//전달받은 글 내용을 미리 준비한 모달창에 뿌린다.
			//값을 제 위치에 배치하고 모달을 연다.(jquery)
		...
		//비동기 방식으로 삭제 진행(버튼 여러개)
		//서버쪽에서 권한을 확인 (작성자와 로그인 중인 사용자의 id를 비교해서 일치하는지의 여부)
			...
			fetch('${pageContext.request.contextPath}/snsboard/' + bno, {
				...
				.then(result => {
					if(result === 'noAuth') alert('권한이 없습니다!');
						...
						getList(1, true);	//삭제가 반영된 글 목록 보여주기
					}
				});
		});
		...
		/*
		무한 스크롤 페이징
		모든 게시판에 무한 스크롤 페이징 방식이 어울리는게 아니다.
